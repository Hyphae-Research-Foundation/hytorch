#!/usr/bin/env bash
# d10 a64 (60M), take 3. Catalog FIRST (the arm with compile risk: two-stage top-M + flat apply/clip,
# commits dbc166e + 45dcc17), then vanilla (its 4 big NEFFs are cached from run-d10 / run-d10b).
# Waits for vanilla2's in-flight compile to land in the cache before stopping it (by PID, no pkill -f).
source /opt/aws_neuronx_venv_pytorch_2_9/bin/activate
cd /opt/trn
B_PID=39401; TR_PID=59618
NEFF=/var/tmp/neuron-compile-cache/neuronxcc-2.26.6360.0+6f180f47/MODULE_5664936211998100556+839ddec0/model.neff
while kill -0 $TR_PID 2>/dev/null && [ ! -f "$NEFF" ]; do sleep 60; done
echo "vanilla2 in-flight NEFF present=$([ -f $NEFF ] && echo yes || echo no) at $(date -u +%FT%TZ); stopping vanilla2"
...
mkdir -p /opt/hyt/d10/hyphae3 /opt/hyt/d10/spool3
...
env HYTORCH_CATALOG=1 HYTORCH_ROOT=/opt/hytorch HYTORCH_D_SLOT=10 HYTORCH_NF=32768 HYTORCH_K=8 HYTORCH_M_CANDIDATES=32 HYTORCH_POLICY_ID=7 HYTORCH_PROPOSE_TOPK=two_stage \
  HYTORCH_MANIFEST=/opt/hyt/manifest-d10.json HYTORCH_BUILD_FACTS=/opt/hyt/build-facts.json HYTORCH_DATA_DIR=/opt/hyt/d10/hyphae3 HYTORCH_SPOOL=/opt/hyt/d10/spool3 \
  ...
  torchrun --standalone --nproc_per_node=4 train.py $COMMON > /opt/hyt/d10/catalog3.log 2>&1
echo "catalog rc=$? $(date -u +%FT%TZ)"
echo "=== ARM vanilla $(date -u +%FT%TZ)"
torchrun --standalone --nproc_per_node=4 train.py $COMMON > /opt/hyt/d10/vanilla3.log 2>&1
